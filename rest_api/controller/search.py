...
elastic_retriever = ElasticsearchRetriever(document_store=document_store)
reader = FARMReader(
    model_name_or_path="ricardo-filho/bert_base_faquad", use_gpu=True)
PIPELINE = ExtractiveQAPipeline(reader, elastic_retriever)
print("PASS")
'''
...
def short_answers(answers):
    for i, answer in enumerate(answers['answers']):
        print('Answer no. {}: {}, with {} percent probability'.format(
            i, answer['answer'], round(answer['probability'] * 100)))
